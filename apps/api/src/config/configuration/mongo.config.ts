...
const mongoSchema = z.object({
  ...
  MONGO_PORT: intFromString(1, 65535).default(DEFAULT_MONGO_PORT),
  ...
  MONGO_SERVER_SELECTION_TIMEOUT_MS: intFromString(100, 120_000).default(
    DEFAULT_MONGO_SELECTION_TIMEOUT_MS,
  ),
  MONGO_SOCKET_TIMEOUT_MS: intFromString(100, 300_000).default(
...
export const mongoConfig = registerAs(MONGO_CONFIG_NAMESPACE, () => {
  ...
  return {
    ...
    serverSelectionTimeoutMS: parsed.MONGO_SERVER_SELECTION_TIMEOUT_MS,
    socketTimeoutMS: parsed.MONGO_SOCKET_TIMEOUT_MS,
  };
});
